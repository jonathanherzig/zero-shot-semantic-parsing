...
class NeuralModel(object):
  ...
  def __init__(self, spec, distract_num=0, float_type=numpy.float64):
    ...
    self.domain_stats_vocab = spec.domain_stats_vocab
  ...
  def train(self, dataset, eta=0.1, T=[], verbose=False, dev_data=None,
            ...
            concat_num=1, concat_prob=0.0, augmenter=None, aug_frac=0.0, evaluator=None):
    ...
    for it in range(max_iters):
      ...
      if evaluator is not None and it>3 and ((it+1) % 5 == 0 or (it+4) % 5 == 0):
        evaluator.evaluate('dev', self, dev_data)
        evaluator.write_stats(it)
